...
export default function QuoteForm({ defaultRegion = 'AU' }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await fetchQuote(region, symbol.trim().toUpperCase());
      setQuote(result);
    } catch (err) {
      ...
      {quote && (
        <div style={{ marginTop: 12, padding: 10, background: '#f9fafb', borderRadius: 6, border: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{quote.region}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <div className="stat" style={{ flex: 1 }}>
              <div className="val" style={{ fontSize: 14 }}>{fmt(quote.open)}</div>
              <div className="lbl">Open</div>
            </div>
            <div className="stat" style={{ flex: 1 }}>
              <div className="val" style={{ fontSize: 14 }}>{fmt(quote.high)}</div>
              <div className="lbl">High</div>
            </div>
            <div className="stat" style={{ flex: 1 }}>
              <div className="val" style={{ fontSize: 14 }}>{fmt(quote.low)}</div>
